...
This is an extension of suit-tool reference implementation developed by ARM. The original forked code is being developed at: https://gitlab.arm.com/research/ietf-suit/suit-tool. 

# Content of the repo

This repository contains a tool to generate and sign manifests following the specification in https://tools.ietf.org/html/draft-ietf-suit-manifest-14. It also include the possibility to parse the manifest into human readable formants and verify the signature. 

The repository is organized as follow:
* the `bin` folder contains the needed file to install the tool and a small example to test the functionalities;
* the `example` folder contains a set of example used for testing and a script to generate and sign the manifests starting from the examples;
* the `suit-tool` folder contains the code of the tool;
* the `test` folder contains the result of the tested example of a Raspberry Pi 4b. 
...
```sh
suit-tool verify -k FILE -m MANIFEST -s SIGNED-MANIFEST
```