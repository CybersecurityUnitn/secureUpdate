...
uint8_t TA_CROSSCON_VALIDATE_MANIFEST(const uint8_t *manifest, size_t manifest_size) {
    ...
    for (size_t i = 0; i < property_ids_count; i++) {
        ...
        if (property_ids[i].locality_constraint == 1) {
            ...
            fseek(f, 0, SEEK_SET);
            uint8_t *proof = malloc(proof_size * sizeof(uint8_t) + 1);
            if (proof == NULL) {
            ...
            // Null-terminate the proof buffer
            proof[read] = '\0';
            //printf("Read value: %d\n", read);

            //printf("Proof Certificate: %s\n", proof);

            // Decode the proof certificate from base64
            int8_t *decoded = base64_decode(proof);
            if (decoded == NULL) {
                printf("Failed to decode proof certificate\n");
                free(proof);
                return ERROR_INVALID_PROOF;
            }
            ...
            size_t decoded_size = strlen(proof) * 3 / 4; // Adjust if the decoder provides the size
            fwrite(decoded, sizeof(__uint8_t), decoded_size, f);
            fclose(f);
            ...
            snprintf(command, sizeof(command), "gunzip -c -q %s > %s", filename, uncompressed_filename);
            printf("Unzipping proof certificate\n");
            int status = system(command);
            //if (status != 0) {
            //    printf("Error unzipping proof certificate %d\n", status);
            //    return ERROR_INVALID_PROOF;
            //}
